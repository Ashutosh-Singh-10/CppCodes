// This code belongs to Ashutosh Singh
...
{   
    ...
    cin>>n;
    map<ll,ll> mp;
    vector<ll> vec,ans(n,LONG_LONG_MAX);
    loop(i,0,n)
    {
    ...
    ll sum=0,cnt=0,prev=LONG_LONG_MIN;
    ...
    loop(k,0,n)
    {
        ...
        {
            auto it=mp.upper_bound(sum-i-1);
            ans[k]=max(cnt+1,k-it->second);
        }
        sum+=i;
        mp[sum]=k;
        if(prev==i)
   ...
   roop(k,n,0)
    {
        ...
        else if(sum>i && cnt!=n-k-1 )
        {
            auto it=mp.upper_bound(sum-i-1);
            ans[k]=min(ans[k],max(cnt+1,it->second-k));
        }
        sum+=i;
        mp[sum]=k;
        if(prev==i)
