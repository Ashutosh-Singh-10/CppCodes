...
void solve()
{
    string s;
    cin>>s;
    HashedString * a=new HashedString(s);
    reverse(s.begin(),s.end());
    HashedString * b=new HashedString(s);
    reverse(s.begin(),s.end());
    ll n=s.length();
    int start=0,end=n-1;
    while(start<end)
    ...
    for(int i=2;i<n-2;i++)
    {
        int lft=(i-1)/2;
        int rt=(n-i-2)/2;
        if(a->get_hash(0,lft) != b->get_hash(n-1-i,n-1-i+lft)  && b->get_hash(0,rt) != a->get_hash(n-1-i,n-1-i+rt))
        {
            cout<<"Yes\n";
            cout<<"2\n";
            cout<<s.substr(0,i+1)<<" ";
