...
void dfs(vector<vector<ll>> &gr,ll cur,ll prev,vector<ll> &colors,ll &ans,map<ll,ll> &mp)
{


    ll last=0;
    if(mp.count(colors[cur]))
    {
        ans+=mp[colors[cur]];
        last=mp[colors[cur]];
    ...
    for(auto &i:gr[cur])
    {
        ...
        {
            mp[colors[cur]]=1;
            dfs(gr,i,cur,colors,ans,mp);

            
        }

    }
    mp[colors[cur]]=last+1;
