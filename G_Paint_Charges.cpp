...
ll n,t;
    cin>>n;
    vector<ll> vec,help(n+1,INT_MAX);
    help[0]=0;
    loop(i,0,n)
    {
        cin>>t;
    ...
    loop(i,1,n+1)
    {
        ...
        for(ll j=min(n,el+i-1);j>i;j--)
        {
            help[j]=min(help[j],mn+1);
            mn=min(help[j],mn);

        }
        mn=LONG_LONG_MAX;
        
        for(ll j=max((ll)0,i-el);j<i;j++)
        {
            mn=min(mn,help[j]);
        }
        help[i]=min(help[i],mn+1);

        
    }
    for(auto &i:help) cout<<i<<" ";cout<<endl;
    cout<<help[n]<<endl;
