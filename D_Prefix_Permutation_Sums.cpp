...
int main()
{
    ...
    while(t--)
    {
        if(solve()) cout<<"YES";else cout<<"NO";cout<<endl;
    }
    return 0;
}
bool solve()
{
    ...
    if(bit) return 0;
    ll cnt=0;
    if( (n*(n+1)) /2<prev) return 0;
    if( (n*(n+1)) /2!=prev)
    {
        st[(  (n*(n+1)) /2)-prev]++;
    
        if(st.size()!=n) return 0;
        for(auto &i:st) if(i.second>1) return 0;
        return 1;
    }
    ...
    for(auto &i:st)
    {
        ...
        if(i.first>n && i.second>1) return 0;
    ...
    for(int i=1;i<n;i++)
    {
        if(st.count(i)==0 && st.count(ex-i)==0 && ex-i<=n) return 1;

    }
    if(ex==n && st.count(n)==0   ) return 1;
    return 0;
