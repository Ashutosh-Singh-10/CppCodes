...
long long sqrti(long long x) // round down
{
    long long a = x, b = ((x + 1) >> 1);
    while (a > b)
    {
        a = b;
        b = ((b + x / b) >> 1);
...
#define gh(index,se)  for (int index = namper / 2 - 1, se = namper - 1; index >= 0 && se >= namper / 2; index--, se--)
//change skr de
bool Allsameamper()
{
...
//delc rk ede
  ...
  //delc rk eee
  ll mxamper = max(aamper[namper-1],bamper[namper-1]);
 
 
  //delc rk eee
 
  ll mnamper = min(aamper[namper-1],bamper[namper-1]);
  ...
  go(index,namper-1)
  {
      ...
      //changes kr de
    ll mx1amper = max(aamper[index],bamper[index]);
 
 
 
    //change skr dee
    ll mn1amper = min(aamper[index],bamper[index]);
 
 
 
    //conlgaa dee
    if(mx1amper>mxamper or mn1amper>mnamper)
    {
...
#define goe1(i,n) for(int i=1;i<=n;++i)
